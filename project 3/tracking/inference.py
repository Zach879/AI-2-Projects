...
def constructBayesNet(gameState: hunters.GameState):
    ...
    OBS0 = "Observation0"
    OBS1 = "Observation1"
    X_RANGE = gameState.getWalls().width #zero to X_RANGE horizontal grids.
    Y_RANGE = gameState.getWalls().height #zero to Y_RANGE verticle grids.
    MAX_NOISE = 7

    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(PAC, OBS0), (GHOST0, OBS0), (PAC, OBS1), (GHOST1, OBS1)] #initialize according to Figure 2.
    variableDomainsDict = {}

    #possible positions a ghost or pacman could be (domain of coordinates).
    ...
    #possible noisy distances from zero to max.
    maxTrueDistance = X_RANGE + Y_RANGE - 2 #longest possible manhattan distance on the grid, -1 for both x and y coordinates.
    ...
    net = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict) #construct Bayes Net structure.
    return net
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #get all factors from the Bayesnet that have evidence from query.
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for var in eliminationOrder: #each var is a hidden variable, which are those not in queryVariables or evidenceDict.
            factors_not_to_join, joinedFactor = joinFactorsByVariable(factors, var)

            if len(joinedFactor.unconditionedVariables()) > 1:
                newFactor = eliminate(joinedFactor, var)
                factors = factors_not_to_join + [newFactor]
            else: #one unconditional variable case.
                factors = factors_not_to_join

        #join all remaining factors after eliminating hidden variables, then normalize probabilities to sum to 1.
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        total_val = self.total()
        r = random.random() * total_val #a random number between 0 and total_val.
        sum = 0.0
        for key, value in self.items():
            sum += value
            if sum >= r: #sum represents the current key in the list of items when choosing a random between 0 and its max (total_val).
                return key
        return list(self.keys())[-1] #return the last key in dictionary if nothing was returned
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        if ghostPosition == jailPosition: #special case
            return 1.0 if noisyDistance is None else 0.0
        if noisyDistance is None: #if still None, then impossible
            return 0.0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        #b’(p) = b(p) * P(observation | pacmanPosition,p,jailPosition)/ normalization, 
        # where b is the current belief distribution, and p is a ghost position.
        for pos in self.allPositions:
            likelihood = self.getObservationProb(observation, pacmanPosition, pos, jailPosition)
            #applying Bayes' rule for b(p) * P(observation | pacmanPosition,p,jailPosition), 
            # where b(p) is self.beliefs[pos] and P(observation | pacmanPosition,p,jailPosition) is likelihood.
            self.beliefs[pos] *= likelihood 
        self.beliefs.normalize() #finishing Bayes' rule calculation by normalizing the result
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        newBeliefs = DiscreteDistribution()  #new belief distribution over legal positions.

        for oldPos in self.allPositions: #this loops over all legal positions.
            newPosDist = self.getPositionDistribution(gameState, oldPos) #ghost's transition distribution, for each legal position.

            for newPos, transProb in newPosDist.items(): #for every possible new ghost position given oldPos on the game grid, add the weighted probabilities.
                newBeliefs[newPos] += self.beliefs[oldPos] * transProb
                
        self.beliefs = newBeliefs #update the belief state with the new probability distribution.
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % numPositions]) #use mod to evenly distribute number of particles among the number of positions.
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles: #count the number of times each unique particle appears in the particle list.
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        #create and populate a DiscreteDistribution to accumulate particle weights.
        weightDistribution = DiscreteDistribution()
        for particle in self.particles:
            weight = self.getObservationProb(observation, pacmanPosition, particle, jailPosition) #compute the likelihood of the observation.
            weightDistribution[particle] += weight
    
        #special case: if all particle weights are zero, reinitialize the particles. shown when pacman exhausts all options.
        if weightDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else: #otherwise, repopulate the particle list with particles sampled based on their weight.
            newParticles = []
            weightDistribution.normalize() #not necessary, but makes sampling easier.
            for i in range(self.numParticles):
                newParticle = weightDistribution.sample()
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        transitionCache = {} #dictionary to cache calls to getPositionDistribution for varying old particle positions.

        for oldParticle in self.particles:
            if oldParticle not in transitionCache: #if doesn't exist in cache, add to cache before using.
                transitionCache[oldParticle] = self.getPositionDistribution(gameState, oldParticle)
            newPosDist = transitionCache[oldParticle] #populate from cache dictionary.

            #sample a new position to replace oldParticle from the transition distribution of oldParticle.
